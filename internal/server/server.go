...
// New starts the ePOS server on the given port. The optional register functions add their own
// routes (the web interface) to the same application, and so to the same port.
func New(port int, mgr *printer.Manager, register ...func(fiber.Router)) *Server {
	...
	for _, registerRoutes := range register {
		registerRoutes(app)
	}
