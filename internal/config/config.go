...
type AppConfig struct {
	...
	Odoo            OdooLink `json:"odoo"`
}

// OdooLink is the pairing with Odoo: set from the web interface and kept across restarts.
// Identifier is generated by this device on first start and never changes.
type OdooLink struct {
	URL        string `json:"url,omitempty"`
	Token      string `json:"token,omitempty"`
	Identifier string `json:"identifier,omitempty"`
	Name       string `json:"name,omitempty"`
	Mode       string `json:"mode,omitempty"` // "local" or "agent"
}
...
// Identifier returns the device id, creating it on first use.
func (cm *Manager) Identifier() string {
	cm.mu.Lock()
	defer cm.mu.Unlock()
	if cm.Data.Odoo.Identifier == "" {
		cm.Data.Odoo.Identifier = uuid.NewString()
		if err := cm.saveLocked(); err != nil {
			log.Printf("[config] warning: could not save: %v\n", err)
		}
	}
	return cm.Data.Odoo.Identifier
}

func (cm *Manager) OdooLink() OdooLink {
	cm.mu.RLock()
	defer cm.mu.RUnlock()
	return cm.Data.Odoo
}

// SetOdooLink saves the pairing (or clears it when empty).
func (cm *Manager) SetOdooLink(link OdooLink) error {
	cm.mu.Lock()
	defer cm.mu.Unlock()
	link.Identifier = cm.Data.Odoo.Identifier
	cm.Data.Odoo = link
	return cm.saveLocked()
}
