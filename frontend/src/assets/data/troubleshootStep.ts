...
export function getWindowsSteps(info: main.TroubleshootInfo) {
  return [
    ...defaultSteps(info),
    {
      ...
      codes: [`New-NetFirewallRule -DisplayName "dooprint" -Direction Inbound \`\n  -Program "${info.execPath}" \`\n  -Action Allow -Profile Private`,],
    },
    ...networkSteps(info),
  ];
}
...
export function getMacSteps(info: main.TroubleshootInfo) {
  return [
    ...defaultSteps(info),
    {
      ...
      desc: "macOS may block incoming network access to dooprint through its built-in Application Firewall.\n\nYou can allow it in *System Settings → Privacy & Security → Firewall*, or run this in Terminal:",
      codes: [`sudo /usr/libexec/ApplicationFirewall/socketfilterfw --unblockapp "/Applications/dooprint.app"`,],
    },
    ...networkSteps(info),
  ];
}
